...
class first(Frame):
    ...
    def load_fileP(self):
        fname = askopenfilename(filetypes=(("shp", "*.shp"),("All files", "*.*") ))
        global SegmentationFile
        SegmentationFile=fname
        if fname:
            try:
...
class AddErrors(Frame):
    ...
    def Done(self):
        ...
        global TableSave
        self.master.destroy()
        #[path1,file1,BAND_number,dataset_number,resolution,is_Spatial,is_Spectral,expected_spatial_error,expected_orientation_error,expected_spectral_error,ismap]
...
class SaveTable(Frame):
    ...
    def first_ok(self):
        ...
        TableSave.to_csv(self.e3.get(),index=False)
        self.master.destroy()
    def file_save(self):
...
if __name__ == "__main__":
    ...
    root = Tk()
    app = AddErrors(root)
    app.master.title('Remote Sensing')
    app.mainloop()  
#
...
if __name__ == '__main__':
    RStable=sorted(RStable,key=lambda l:-float(l[4]))
    global Lowest_resolution
    Lowest_resolution=float(RStable[0][4])
    print('Lowest_resolution',Lowest_resolution)
...
if __name__ == '__main__':
    ...
    for fd1 in field_data:
        i+=1
        for rs in RStable:
            FsError.subset_image(extend_image=fd1[0]+fd1[1], inDS=rs[0]+rs[1], outDS=rs[0]+'subset/'+str(i)+'/'+rs[1],tolerance=2*max([float(fd1[7]),float(rs[7])]))#'''
            if int(fd1[-1])==1:
              FsError.subset_image(extend_image=fd1[-2], inDS=rs[0]+rs[1], outDS=rs[0]+'subset/'+str(i)+'/error'+rs[1],tolerance=2*max([float(fd1[7]),float(rs[7])]))#'''  
if __name__ == '__main__':
    global tables
...
if __name__ == '__main__':
    ...
    p.close()
    p.join()
error_pridiction_tool(tables[0])
...
#'''
#SquareRoot(Square(2*"band6"/Square("band6"+"band4")*Square(0.1*"band6"))+Square((2*"band4"/Square("band6"+"band4"))*Square(0.1*"band4")))